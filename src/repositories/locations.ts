...
export class LocationsRepository implements IRepository<Location, number> {
  public async find(id: number): Promise<Location> {
    ...
    const result: { index: number; obj: Location } = BinarySearchAlgorithm.search<Location>(
      locations,
  ...
  protected getLocations(): Location[] {
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      locations.push(
        new Location(index, parseFloat(columns[2]), parseFloat(columns[3]), `${columns[0]}, ${columns[5]}`),
      );
    }
